...
console.log("const ARIA_PROP_TYPES: Map<&'static str, AriaPropType> = phf_map! {");
for (prop of properties) {
    ...
    console.log(`   "${prop_name}" => ${map_type(conf.type)} },`);
}
console.log("};\n");

console.log("const ARIA_ALLOW_UNDEFINED: Set<&'static str> = phf_set! {");
for (prop of properties) {
    const prop_name = prop[0];
    const conf = prop[1];
    if (conf.allowundefined) {
        console.log(`    "${prop_name}", `);
    }
}
console.log("};\n");

console.log("const ARIA_PROP_VALUES: Map<&'static str, Set<&'static str>> = phf_map! {");
for (prop of properties) {
    const prop_name = prop[0];
    const conf = prop[1];
    if (conf.values) {
        console.log(`    ${prop_name} => phf_set! { ` + conf.values.filter(val => typeof val !== "boolean").map(val => `"${val}"`).join(", ") + "})");
    }
}
console.log("};\n");

console.log("const ARIA_PROP_ALLOW_BOOLEAN_VALUES: Set<&'static str> = phf_set! {");
for (prop of properties) {
    const prop_name = prop[0];
    const conf = prop[1];
    if (conf.values && conf.values.some(val => typeof val === "boolean")) {
        console.log(`    "${prop_name}",`);
    }
}
console.log("};\n");
